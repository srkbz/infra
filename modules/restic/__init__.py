...
_archive_url = f"https://github.com/restic/restic/releases/download/v{VERSION}/restic_{VERSION}_{VARIANT}.bz2"
_archive_sums = join(dirname(__file__), "versions", VERSION, "SHA256SUMS")
_bin = join(_version_dir, f"restic_{VERSION}_{VARIANT}")
...
def install(dry_run: bool):
    ...
    if not isfile(_archive_verified):
        assert not dry_run
        shell(f"cat '{_archive_sums}' | grep '{VARIANT}' | sha256sum --check")
        shell(f"touch '{_archive_verified}'")
    if not isfile(_bin):
        assert not dry_run
        shell(
            f"bzip2 -d 'restic_{VERSION}_{VARIANT}.bz2'",
